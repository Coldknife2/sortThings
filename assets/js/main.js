...
function displayResults()
{
    ...
    let result = destinationArray.map((value, index) => {
        ...
        return toAppend.replace("{#Number#}", index + 1).replace("{#Text#}", escapeHtml(value));
    });
...
<div class="inline-block pr-2 mr-2 border-r-2 border-black"><svg class="w-12" x="0px" y="0px" viewBox="0 0 130 130"><g><path
    ...
    fill="#ffcd00" fill-opacity="1" fill-rule="evenodd" stroke="#000000" stroke-width="4.33534527" stroke-linecap="butt" stroke-linejoin="miter" stroke-miterlimit="4" stroke-opacity="1"/></g></svg></div>
<div class="inline-block whitespace-pre-wrap">{#Text#}</div></div>`;
...
document.getElementById("choiceTextLeft").addEventListener("click", left);
document.getElementById("choiceTextRight").addEventListener("click", right);
